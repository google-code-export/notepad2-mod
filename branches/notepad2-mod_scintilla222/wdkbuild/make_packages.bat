...
START "" /B /WAIT "%TOOLS_PATH%\7za.exe" a -tzip -mx=9^
 "Notepad2-mod.%NOTEPAD_VERSION%_r%VerRev%_%2_%PREFFIX%.zip" "License.txt" "Notepad2.exe"^
 "Notepad2.ini" "Notepad2.txt" "ReadMe.txt" >NUL
IF %ERRORLEVEL% NEQ 0 GOTO :ErrorDetected

ECHO:Notepad2-mod.%NOTEPAD_VERSION%_r%VerRev%_%2_%PREFFIX%.zip created successfully!
MOVE /Y "Notepad2-mod.%NOTEPAD_VERSION%_r%VerRev%_%2_%PREFFIX%.zip" "..\packages" >NUL 2>&1
ECHO.
POPD
...
MOVE setup.x86-32\addon.7z      ..\wdkbuild\packages\Notepad2-mod_Addon.x86-32_%PREFFIX%.7z >NUL
...
MOVE setup.x86-64\addon.7z      ..\wdkbuild\packages\Notepad2-mod_Addon.x86-64_%PREFFIX%.7z >NUL
