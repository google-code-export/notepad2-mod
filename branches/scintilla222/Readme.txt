...
Rebuilding from the Source Code
  ...
  Notepad2 4.1.24-mod has been created with Scintilla 2.22. The only
  modification in Scintilla source code is to comment out the unused lexers
  in src\Catalogue.cxx otherwise you'll get unresolved external symbols when
  linking. That's because we don't need all the lexers so we don't need to 
  build them all.
